...
void delete(struct node* k,struct node* r,int f)
{
    int d,c;
    struct node* m;
    if(root==NULL)
    ...
    else
    {
        ...
        if(k->left!=NULL)
        {
            ...
            d=m->data;
            //printf("\n %dhello ",m->data);
            delete(m,root,0);//if i write k->data=m->data before this line then root changes and when calling delete()there come == condition and goes to else 
